...
import { useAsamAssessmentList } from "@/features/shared/services/queries";
import { keepPreviousData, useQueryClient } from "@tanstack/react-query";
import { queryKeys } from "@/lib/queryKeys";
const AsamAssessment = () => {
  ...
  const { data: response, isLoading: loading } = useAsamAssessmentList(
    { page, limit },
  ...
  const fetchHandler = useCallback(() => {
    queryClient.invalidateQueries({
      queryKey: queryKeys.patientChart.list("asam"),
    });
  }, [queryClient]);
            ...
            <InDraft
              link={
                userType === ROLES.PATIENT
                  ? `/update-asam-assessment-resident/${item._id}`
                  : `/update-asam-assessment/${item._id}`
              }
            />
          ) : (
            <DropList
              viewLink={
                userType === ROLES.PATIENT
                  ? `/view-asam-assessment-resident/${item._id}`
                  : `/view-asam-assessment/${item._id}`
              }
              editLink={
                userType === ROLES.PATIENT
                  ? `/update-asam-assessment-resident/${item._id}`
    ...
    <>
      ...
      <DeleteDocModal
        ...
        onDelete={({ additionalFunctions }) =>
          patientChartService.asam.remove({ id: deleteId, additionalFunctions })
        }
      ></DeleteDocModal>
