...
import { useDischargePlanningList } from "@/features/shared/services/queries";
import { keepPreviousData, useQueryClient } from "@tanstack/react-query";
import { queryKeys } from "@/lib/queryKeys";

const DischargePlannning = () => {
  ...
  const { data: response, isLoading: loading } = useDischargePlanningList(
    { page, limit },
  ...
  const fetchHandler = useCallback(() => {
    queryClient.invalidateQueries({
      queryKey: queryKeys.patientChart.list("dischargePlanning"),
    });
  }, [queryClient]);
            ...
            <InDraft
              link={
                userType === ROLES.PATIENT
                  ? `/update-discharge-planning-resident/${item._id}`
                  : `/update-discharge-planning/${item._id}`
              }
            />
          ) : (
            <DropList
              viewLink={
                userType === ROLES.PATIENT
                  ? `/view-discharge-planning-resident/${item._id}`
                  : `/view-discharge-planning/${item._id}`
              }
              editLink={
                userType === ROLES.PATIENT
                  ? `/update-discharge-planning-resident/${item._id}`
  ...
  return (
    <>
      <CreateNav
        ...
        link={"/create-discharge-planning"}
      ...
      <DeleteDocModal
        ...
        onDelete={({ additionalFunctions }) =>
          patientChartService.dischargePlanning.remove({
            id: deleteId,
            additionalFunctions,
          })
        }
      ...
      <Container className="full-width-container discharge-summary-cell-ellipsis">
        <TableLayout
          thead={[`Resident Name`, "Admit Date", "Date of Discharge", "Action"]}
          tbody={dataList}
          loading={loading}
        />
...
export default HOC({
  Wcomponenet: DischargePlannning,
});
