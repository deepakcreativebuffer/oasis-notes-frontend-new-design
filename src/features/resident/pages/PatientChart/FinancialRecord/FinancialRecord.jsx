...
import { useFinancialRecordList } from "@/features/shared/services/queries";
...
const FinancialRecord = () => {
  ...
  const { data: response, isLoading: loading } = useFinancialRecordList(
    { page, limit },
  ...
  const fetchHandler = useCallback(() => {
    queryClient.invalidateQueries({
      queryKey: queryKeys.patientChart.list("financialRecord"),
    });
  }, [queryClient]);
  ...
  const dataList =
    response?.docs?.length > 0
      ? [...response?.docs]?.map((item) => [
          item?.residentName || fetchPaitentName(item?.patientId),
          formatDateToMMDDYYYY(item?.patientId?.admitDate),
          formatDateToMMDDYYYY(item?.dateOfBirth),
          item?.saveAsDraft ? (
            <InDraft link={`/edit-record/${item._id}`} />
          ) : (
            <DropList
              viewLink={`/view-record/${item._id}`}
              editLink={`/edit-record/${item._id}`}
    ...
    <>
      ...
      <DeleteDocModal
        ...
        onDelete={({ additionalFunctions }) =>
          patientChartService.financialRecord.remove({
            id: deleteId,
            additionalFunctions,
          })
        }
      ...
      <Container className="full-width-container">
        <TableLayout
          thead={["Resident Name", "Admit Date", "Date of Birth", "Action"]}
          tbody={dataList}
          loading={loading}
        />
        {dataList?.length > 0 && (
