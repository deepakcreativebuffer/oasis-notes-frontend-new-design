...
import { formatDateToMMDDYYYY, ronSignerLineFromProfile } from "@/utils/utils";
...
const UpdateRecertificationOfNeed = () => {
  ...
  useEffect(() => {
    const row = data?.data;
    if (!row) return;
    const pid = row.patientId?._id || row.patientId;
    setPatientId(pid || "");
    ...
    setBhpSignatureDateTime(row.bhpSignatureDateTime || "");
    ...
    setSaveAsDraft(!!row.saveAsDraft);
    const cn =
      `${row.memberFirstName || row.patientId?.firstName || ""} ${row.memberLastName || row.patientId?.lastName || ""}`.trim() ||
      row.clientName ||
      "";
    setClientName(cn);
    const fromPatient = row.patientId;
    if (
      (!row.memberAhcccsId || row.memberAhcccsId === "") &&
      fromPatient?.ahcccsId
    ) {
      setMemberAhcccsId(fromPatient.ahcccsId);
    }
    if ((!row.memberDob || row.memberDob === "") && fromPatient?.dateOfBirth) {
      setMemberDob(fromPatient.dateOfBirth);
    }
    if (
      (!row.memberPrimaryIcd10 || row.memberPrimaryIcd10 === "") &&
      fromPatient?.diagnosis
    ) {
      setMemberPrimaryIcd10(fromPatient.diagnosis);
    }
    if ((!row.memberPhone || row.memberPhone === "") && fromPatient?.phone) {
      setMemberPhone(fromPatient.phone);
    }
    if ((!row.memberEmail || row.memberEmail === "") && fromPatient?.email) {
      setMemberEmail(fromPatient.email);
    }
    if (
      (!row.memberFirstName || row.memberFirstName === "") &&
      fromPatient?.firstName
    ) {
      setMemberFirstName(fromPatient.firstName);
    }
    if (
      (!row.memberLastName || row.memberLastName === "") &&
      fromPatient?.lastName
    ) {
      setMemberLastName(fromPatient.lastName);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [data?.data?._id, loadFromApi, profile]);
  const checkSign = useCallback(() => {
    const signersRows = signers?.length ? signers : data?.data?.signers;
    let signerIndex = signersRows?.findIndex?.(
      (signer, i) => String(signer.signerId) === String(profile._id),
    );
    let isSignerValid =
      signerIndex !== -1 && signersRows?.[signerIndex]?.signature?.length > 0;
    let isAdminConditionValid = profile.userType === ROLES.ADMIN;
    const hasPen = !!signatures?.bhp?.rawSignatureImage;
    let isEmployeeConditionValid =
      (data?.data?.employeeId === profile?._id ||
        data?.data?.employeeId?._id === profile?._id) &&
      ((bhpSignature?.length ?? 0) > 0 || hasPen);
    let signerGuadianIndex = signersRows?.findIndex?.((signer, i) =>
      profile.patientsAssigned?.includes(signer.signerId),
    );
    let isGuadianConditionValid =
      signerGuadianIndex !== -1 &&
      signersRows?.[signerGuadianIndex]?.signature?.length > 0;
    if (
      isSignerValid ||
  ...
  const initialFormData = {
    patientId: patientId || data?.data?.patientId?._id || data?.data?.patientId,
  ...
  const submitHandler = (e) => {
    e.preventDefault();
    patientChartService.recertificationOfNeed.update(
      id || data?.data?._id,
      initialFormData,
      { setLoading, navigate },
    );
      ...
      <Container>
        <Form onSubmit={submitHandler}>
          <div className="ron-print-root">
            ...
            <Card body className="mb-2 mb-md-3 hidePrint">
              <Row>
                <Col xs={12} md={6}>
            ...
            <Card body className="mb-3 ron-print-avoid">
              ...
              <p className="small">
                Specify signs and symptoms from the diagnosed behavioral health
                condition necessitating continuation in IOP, criteria met per
                AMPM 320-V, and EBPPs used to determine medical necessity.
              </p>
              <Form.Control
                as="textarea"
                rows={8}
            ...
            <Card body className="mb-3 ron-print-avoid">
              ...
              <p className="small">
                Specify signs and symptoms necessitating BHRF continuation and
                criteria met per AMPM 320-V.
              </p>
              <Form.Control
                as="textarea"
                rows={8}
                ...
                <button
                  ...
                  disabled={
                    profile.userType === ROLES.ADMIN
                      ? false
                      : !(gateOkForSubmit && (saveAsDraft || isSubmitEnabled))
                  }
                >
...
export default HOC({
  Wcomponenet: UpdateRecertificationOfNeed,
});
