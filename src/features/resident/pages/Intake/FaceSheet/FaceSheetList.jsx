...
import { useFaceSheetList } from "@/features/shared/services/queries";
...
const FaceSheetList = () => {
  ...
  const { data: response, isLoading: loading } = useFaceSheetList(
    { page, limit },
  ...
  const fetchHandler = useCallback(() => {
    queryClient.invalidateQueries({
      queryKey: queryKeys.intake.list("faceSheet"),
    });
  }, [queryClient]);
  ...
  return (
    <>
      <CreateNav title={"Face Sheet"} link={"/create-face-sheet"} />
      <DeleteDocModal
        show={showDeleteModal}
        handleClose={() => setShowDeleteModal(false)}
        fetchHandler={fetchHandler}
        onDelete={({ additionalFunctions }) =>
          intakeService.faceSheet.remove({ id: deleteId, additionalFunctions })
        }
      ></DeleteDocModal>
      <Container>
        <TableLayout
