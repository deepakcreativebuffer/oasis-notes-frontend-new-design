...
export default function ViewTreatmentPlanCounselingSection() {
  const f = useViewTreatmentPlanFormContext();
  return (
    <>
      {(f.minimumHoure || f.counselingOptions?.length > 0) && (
        <div
          className={`${!f.minimumHoure && f.counselingOptions?.length < 1 && "hide-data-on-view-print"}`}
        >
          <Form.Label className="fw-bold w-100 mt-2 mb-0">
                ...
                <div className="d-block">
                  <div className="radio-inline">
                    ...
                    {f.counselingOptions.includes(
                      ...
                      <span className="view-value">{f.IndividualComment}</span>
                    )}
                  </div>
                </div>
              </div>
