...
const Objective = () => {
  ...
  const { data: objectives, isLoading: loading } = useQuery({
    ...
    queryFn: () =>
      adminDashboardService.treatmentMeasureable.listObjectives({
        page,
        limit,
        debouncedQuery,
      }),
  ...
  useEffect(() => {
    if (objectives?.data?.docs?.length === 0 && page > 1) {
      setPage(page - 1);
    }
  }, [objectives?.data?.docs?.length, page]);
  ...
  const handleMakeClone = function (id) {
    ...
    adminDashboardService.treatmentMeasureable.clone(id, {
      ...
      successMsg: "Treatment Objective Cloned !",
  ...
  function MyVerticallyCenteredModal(props) {
    ...
    const handleSubmit = async (e) => {
      ...
      adminDashboardService.treatmentMeasureable.create(formData, {
        successMsg: "Treatment Objective Created !",
    ...
    const handleSubmit2 = async (e) => {
      ...
      adminDashboardService.treatmentMeasureable.update(editId, payload2, {
        successMsg: "Treatment Objective Updated !",
        additionalFunctions: [getAllnotes, props.onHide],
      });
    };
        ...
        {addContactBtn === "f" ? (
          <>
            <Modal.Header closeButton>
              <h5 className="fw-bold mb-0">View Treatment Objective</h5>
            </Modal.Header>
            <Modal.Body>
                ...
                <Form>
                  <div className="view-details">
                    <Row>
                      ...
                      <Col xs={12} md={12} lg={12}>
                        <div className="view-details-grid view-details-grid-inline my-1 md-2 p-3">
          ...
          <>
            <Modal.Header closeButton>
              <h5 className="fw-bold mb-0">Add Treatment Objective</h5>
            </Modal.Header>
            <Form onSubmit={handleSubmit}>
              <ModalBody>
                <div>
                  ...
                  <Form.Group controlId="objectiveSummary">
          ...
          <>
            <Modal.Header closeButton>
              <h5 className="fw-bold mb-0">Update Treatment Objective</h5>
            </Modal.Header>
  ...
  return (
    <>
      ...
      <Container>
        <Row>
          <Col>
            <div className="page-title-bar">
              <p className="heading">Treatment Objective</p>
            </div>
          </Col>
        </Row>
        <Row className="mt-3">
        ...
        <Table bordered responsive>
          ...
          <tbody>
            {loading ? (
              ...
              objectives?.data?.docs &&
              objectives?.data?.docs?.map((item, index) => {
                return (
                  <tr key={index}>
                    <td>
                      {item.topic
        ...
        {objectives?.data?.docs?.length > 0 && (
          <PaginationsPage
            page={page}
            setPage={setPage}
            totalPages={
              objectives?.data?.totalPages && objectives?.data?.totalPages
            }
            limit={limit}
            setLimit={setLimit}
          />
