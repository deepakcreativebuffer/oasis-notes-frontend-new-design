...
const Interventions = () => {
  ...
  const { data: interventions, isLoading: loading } = useQuery({
    ...
    queryFn: () =>
      adminDashboardService.treatmentMeasureable.listInterventions({
        page,
        limit,
        debouncedQuery,
      }),
  ...
  useEffect(() => {
    if (interventions?.data?.docs?.length === 0 && page > 1) {
      setPage(page - 1);
    }
  }, [interventions?.data?.docs?.length, page]);
  ...
  function MyVerticallyCenteredModal(props) {
    ...
    const handleSubmit = async (e) => {
      e.preventDefault();
      try {
        const res = await adminDashboardService.treatmentMeasureable.create(
          formData,
          {
            additionalFunctions: [getAllnotes],
          },
        );
        if (res && props.onHide) {
          props.onHide();
    ...
    const handleSubmit2 = async (e) => {
      ...
      try {
        await adminDashboardService.treatmentMeasureable.update(
          editId,
          payload2,
          { additionalFunctions: [getAllnotes] },
        );
        ...
        {addContactBtn === "f" ? (
          <>
            <Modal.Header closeButton>
              <h5 className="fw-bold mb-0">View Interventions</h5>
            </Modal.Header>
            <Modal.Body>
              <div ref={componentRef}>
                <h1 className="pdfTitle hidden m-2.5">View Interventions</h1>
                <Form>
                  <div className="view-details">
                    <Row>
                      ...
                      <Col xs={12} md={12} lg={12}>
                        <div className="view-details-grid view-details-grid-inline my-1 my-md-2 p-3">
                          <p className="view-label mb-1">Interventions : </p>
          ...
          <>
            <Modal.Header closeButton>
              <h5 className="fw-bold mb-0">Interventions</h5>
            </Modal.Header>
          ...
          <>
            <Modal.Header closeButton>
              <h5 className="fw-bold mb-0">Update Interventions</h5>
            </Modal.Header>
      ...
      <Container>
        <Row>
          <Col>
            <div className="page-title-bar">
              <p className="heading">Interventions</p>
            </div>
          </Col>
        </Row>
        <Row className="mt-3">
        ...
        <Table bordered responsive>
          ...
          <tbody>
            {loading ? (
            ...
            ) : !interventions?.data?.docs?.length ? (
              <tr>
              ...
              interventions?.data?.docs &&
              interventions?.data?.docs?.map((item, index) => {
                return (
                  <tr key={index}>
                    <td>
                      {item.topic
        ...
        {interventions?.data?.docs?.length > 0 && (
          <PaginationsPage
            page={page}
            setPage={setPage}
            totalPages={
              interventions?.data?.totalPages && interventions?.data?.totalPages
            }
            limit={limit}
            setLimit={setLimit}
          />
