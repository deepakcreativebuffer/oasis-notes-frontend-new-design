...
import { useInfectionControlList } from "@/features/shared/services/queries";
import { keepPreviousData, useQueryClient } from "@tanstack/react-query";
import { queryKeys } from "@/lib/queryKeys";
const GetInfectionControl = () => {
  ...
  const { data: response, isLoading: loading } = useInfectionControlList(
    { page, limit },
      ...
      ? [...response?.docs]?.map((i, index) => {
          const result = [
            ...
            <DropList
              ...
              viewLink={`/infection-control/${i._id}`}
              editLink={`/edit-infection-control/${i._id}`}
              canEdit={
    ...
    <>
      ...
      <DeleteDocModal
        ...
        onDelete={({ additionalFunctions }) =>
          trainingService.deleteInfectionControl({
            id: deleteId,
            additionalFunctions,
          })
        }
...
export default HOC({
  Wcomponenet: GetInfectionControl,
});
