...
import { useApsList } from "@/features/shared/services/queries";
import { keepPreviousData, useQueryClient } from "@tanstack/react-query";
import { queryKeys } from "@/lib/queryKeys";
const AllAps = () => {
  const { userType, accountType, userPermissions, hoursFormat } =
  ...
  const { data: response, isLoading: loading } = useApsList(
    { page, limit },
  ...
  const fetchHandler = useCallback(() => {
    queryClient.invalidateQueries({ queryKey: queryKeys.aps.all() });
  }, [queryClient]);
    ...
    response?.docs?.length > 0
      ? [...response?.docs]
          ...
          ?.map((i) => [
            ...
            i?.saveAsDraft ? (
              <InDraft link={`/edit-aps/${i._id}`} />
            ) : (
              <DropList
                viewLink={`/view-aps/${i._id}`}
                showDelete={
                ...
                editLink={`/edit-aps/${i._id}`}
                deleteLink={() => deleteHandler(i._id)}
                canEdit={
                  ((i?.employeeId?._id || i?.employeeId) === currentUserId &&
  ...
  return (
    <>
      ...
      <Container className="full-width-container">
        <TableLayout
          thead={["Date", "Employee Name", "Employee Signature", "Action"]}
          tbody={dataList}
          loading={loading}
        />
        {dataList.length > 0 && (
